...
class ProventoRepository {
  ...
  // ========== METODOS CRUD COM SINCRONIZACAO ==========
  ...
  Future<int> update(Provento provento) async {
    if (provento.id == null) throw Exception('ID nao pode ser nulo');
  ...
  /// Agenda notificacoes para proventos futuros
    ...
    for (var p in futuros) {
      if (p.id != null) {
        try {
          ...
          debugPrint('Erro ao agendar notificacao para ${p.ticker}: $e');
        }
      }
    }
  }
  ...
  Future<Result<int>> updateProventoResult(Provento provento) async {
    try {
      if (provento.id == null) {
        return Result.failure('ID do provento nao pode ser nulo');
      }
  ...
  Future<Result<List<Provento>>> getByTickerResult(String ticker) async {
    try {
      ...
      return Result.failure('Erro ao buscar proventos por ticker: $ticker\n$e');
    }
  }

  /// Busca proventos por periodo com Result
  Future<Result<List<Provento>>> getByPeriodoResult(
    ...
    try {
      ...
      return Result.failure('Erro ao buscar proventos por periodo: $e');
    }
  }

  /// Calcula estatisticas com Result
