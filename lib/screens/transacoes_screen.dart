...
class Transacao {
  ...
  factory Transacao.fromMap(Map<String, dynamic> map) {
    ...
    final precoMedio = (map['preco_medio'] as num?)?.toDouble() ?? 0;
    return Transacao(
      id: map['id'] as int?,
      remoteId: map['remote_id']?.toString(),
      ticker: map['ticker']?.toString() ?? '',
      tipo: map['tipo_transacao']?.toString() ??
          map['tipo']?.toString() ??
          'COMPRA',
      tipoInvestimento: map['tipo']?.toString() ?? 'ACAO',
      quantidade: quantidade,
      preco: precoMedio,
      taxa: 0,
      valorTotal: quantidade * precoMedio,
      data: DateTime.tryParse(map['data_compra']?.toString() ??
              map['data']?.toString() ??
...
class _TransacoesScreenState extends State<TransacoesScreen> {
  final InvestimentoRepository _repository = InvestimentoRepository();
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text(
                            Formatador.data(dataSelecionada),
                            style: TextStyle(
    ...
    if (result != null && mounted) {
      try {
        ...
        await db.insert('investments', {
          ...
          'tipo': result['tipo_investimento'],
          'tipo_transacao': result['tipo'],
          'quantidade': result['quantidade'],
          'preco_medio': result['preco'],
          'preco_atual': result['preco'],
          'data_compra': result['data'].toString().split('T')[0],
          'user_id': userId,
          'sync_status': 'synced',
        });
      ...
      try {
        ...
        await db.update(
          'investments',
          {
            ...
            'preco_atual': result['preco'],
            'data_compra':
                (result['data'] as DateTime).toIso8601String().split('T')[0],
            'sync_status': 'synced',
          },
          where: 'id = ?',
  ...
  Future<void> _apagarTransacao(Transacao transacao) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Apagar ${transacao.tipoDescricao} de ${transacao.ticker}?',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      } else if (transacao.remoteId != null) {
        await db.delete(
          'investments',
          where: 'remote_id = ?',
          whereArgs: [transacao.remoteId],
        );
      }
      await _carregarTransacoes();
