...
class _InvestimentosScreenState extends State<InvestimentosScreen> {
        ...
        if (!agrupados.containsKey(ticker)) {
          ...
          if (tipo == 'COMPRA') {
            ...
            final novoPrecoMedio =
                (valorTotalAntigo + (quantidade * preco)) / novaQuantidade;
            agrupados[ticker] = Investimento(
                ticker: ticker,
                tipo: tipoInvestimento,
  ...
  List<Map<String, dynamic>> get _dadosDistribuicao {
    ...
    return valores.entries
        ...
        .map((e) => {
              ...
              'valor': e.value,
              'percentual':
                  _patrimonioTotal > 0 ? (e.value / _patrimonioTotal) * 100 : 0
            })
        .toList();
  }
  ...
  Future<void> _adicionarInvestimento(Investimento investimento,
    ...
    try {
      ...
      await db.insert('investments', {
        ...
        'preco_medio': investimento.precoMedio,
    ...
    return Expanded(
        child: Container(
            ...
            decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: cor.withValues(alpha: 0.2), width: 1)),
            child:
    ...
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...top5.asMap().entries.map((e) {
            final idx = e.key;
            final inv = e.value;
            final isPos = inv.variacaoPercentual >= 0;
  ...
  Widget _buildTransacaoCard(Map<String, dynamic> t) {
    ...
    final preco = (t['preco_medio'] as num?)?.toDouble() ?? 0.0;
    final total = quantidade * preco;
    final data = DateTime.parse(
        t['data_compra']?.toString() ?? DateTime.now().toIso8601String());
    return InkWell(
        onTap: () async {
          final result = await Navigator.push(
              context,
                        ...
                        child: Center(
                            child: Text(
                                ...
                                style: TextStyle(
                                    ...
                                    fontWeight: FontWeight.bold,
                                    color:
                                        TipoInvestimento.getCor(inv.tipo))))),
                    const SizedBox(width: 12),
                    Expanded(
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
        ...
        builder: (context) => AlertDialog(
              ...
              title: Row(children: [
                Container(
                    ...
                    child: Center(
                        child: Text(
                            inv.ticker.substring(0,
                                inv.ticker.length > 2 ? 2 : inv.ticker.length),
                            style: TextStyle(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                                color: TipoInvestimento.getCor(inv.tipo))))),
              ...
              content: Column(mainAxisSize: MainAxisSize.min, children: [
                _buildDetalheLinha(
                    ...
                    Icons.numbers),
                _buildDetalheLinha('Preco Medio',
