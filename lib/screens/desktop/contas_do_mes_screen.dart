// lib/screens/desktop/contas_do_mes_screen.dart
...
class _ContasDoMesScreenState extends State<ContasDoMesScreen> {
  ...
  Future<void> _carregarDados() async {
    print('🔵 [DEBUG] _carregarDados INICIADO');
    print(
        '🔵 [DEBUG] mesSelecionado: ${_mesSelecionado.year}/${_mesSelecionado.month}');
    ...
    try {
      ...
      print(
          '🔵 [DEBUG] _carregarDados: ${dados.length} pagamentos encontrados');
      ...
      print('🔴 [DEBUG] _carregarDados ERRO: $e');
    } finally {
      if (mounted) setState(() => _isLoading = false);
      print('🔵 [DEBUG] _carregarDados FINALIZADO');
    }
  }

  Future<void> _carregarParcelasInfo() async {
    print('🔵 [DEBUG] _carregarParcelasInfo INICIADO');
    ...
    print('🔵 [DEBUG] _carregarParcelasInfo FINALIZADO');
  }
  ...
  Widget build(BuildContext context) {
    print('🔵 [DEBUG] build() chamado');
    print('🔵 [DEBUG] _pagamentos.length = ${_pagamentos.length}');
                    ...
                    children: [
                      ...
                      if (isParcelada) ...[
                        ...
                        Container(
                          ...
                          child: Text('${info['atual']}/${info['total']}',
                              style: TextStyle(
                                  fontSize: 9,
                                  fontWeight: FontWeight.bold,
                                  color: AppColors.primary)),
                      ...
                      Container(
                        ...
                        child: Text(isPaid ? 'PAGA' : 'PENDENTE',
                            style: TextStyle(
                                fontSize: 9,
                                fontWeight: FontWeight.bold,
  ...
  Future<void> _pagar(Map<String, dynamic> b) async {
    print('🟡 [DEBUG] _pagar INICIADO');
    print('🟡 [DEBUG] _pagar dados: $b');

    final id = b['id'].toString();
    final res = await _repository.pagarContaComLancamentoString(id);

    print('🟡 [DEBUG] _pagar res.isSuccess = ${res.isSuccess}');

    if (res.isSuccess) {
      setState(() => _contasPagasLocal.add(id));
      try {
        final lancamento = {
  ...
  void _adicionarNovaConta() async {
    print('🟢 [DEBUG] _adicionarNovaConta INICIADO');

    if (!mounted) return;
    final resultado = await AppModals.mostrarModalConta(context: context);

    print('🟢 [DEBUG] _adicionarNovaConta resultado = $resultado');
    ...
    if (resultado != null) {
      ...
      try {
        ...
        _carregarDados();
        Toast.success(context, '✅ Conta adicionada!');
      } catch (e) {
        LoggerService.error('Erro ao salvar conta: $e');
        Toast.error(context, 'Erro ao salvar conta');
      }
    }

    print('🟢 [DEBUG] _adicionarNovaConta FINALIZADO');
  }
  ...
  void _editar(Map<String, dynamic> b) async {
    print('🟣 [DEBUG] _editar INICIADO');
    print('🟣 [DEBUG] _editar dados recebidos: $b');
    print('🟣 [DEBUG] _editar mounted = $mounted');

    if (!mounted) {
      print('🔴 [DEBUG] _editar: mounted é FALSE, saindo!');
      return;
    }

    // 🔍 PEGA O ID DA CONTA
    final contaId = b['conta_id']?.toString() ?? '';
    final remoteId = b['remote_id']?.toString() ?? '';

    print('🟣 [DEBUG] _editar contaId = "$contaId"');
    print('🟣 [DEBUG] _editar remoteId = "$remoteId"');

    // 🔥 PRIORIDADE: conta_id (ID DA CONTA)
    final idParaBuscar = contaId.isNotEmpty ? contaId : remoteId;

    print('🟣 [DEBUG] _editar idParaBuscar = "$idParaBuscar"');

    if (idParaBuscar.isEmpty) {
      print('🔴 [DEBUG] _editar: idParaBuscar VAZIO!');
      Toast.error(context, '❌ ID da conta não encontrado');
      return;
    }

    Toast.info(context, '🔍 Buscando conta...');

    final conta = await _repository.getContaByIdString(idParaBuscar);

    print('🟣 [DEBUG] _editar conta encontrada? ${conta != null}');
    if (conta != null) {
      print('🟣 [DEBUG] _editar conta.nome = "${conta.nome}"');
      print('🟣 [DEBUG] _editar conta.id = "${conta.id}"');
    }

    if (conta == null) {
      print('🔴 [DEBUG] _editar: conta NÃO encontrada!');
      Toast.error(context, '❌ Conta não encontrada');
      return;
    }

    print('🟣 [DEBUG] _editar: Abrindo modal de edição');

    AppModals.mostrarModalConta(
      context: context,
      conta: conta.toJson(),
      onSalvo: () async {
        print('🟣 [DEBUG] _editar: onSalvo chamado!');
        Toast.info(context, '💾 Salvando...');
        await Future.delayed(const Duration(milliseconds: 300));
        print('🟣 [DEBUG] _editar: Chamando _carregarDados()');
        await _carregarDados();
        print('🟣 [DEBUG] _editar: _carregarDados() finalizado');
        Toast.success(context, '✅ Conta atualizada!');
      },
    );

    print('🟣 [DEBUG] _editar FINALIZADO');
  }
  ...
  void _excluir(Map<String, dynamic> b) async {
    print('🔴 [DEBUG] _excluir INICIADO');
    print('🔴 [DEBUG] _excluir dados recebidos: $b');
    print('🔴 [DEBUG] _excluir mounted = $mounted');

    if (!mounted) {
      print('🔴 [DEBUG] _excluir: mounted é FALSE, saindo!');
      return;
    }
    ...
    print('🔴 [DEBUG] _excluir contaId = "$contaId"');
    print('🔴 [DEBUG] _excluir remoteId = "$remoteId"');
    print('🔴 [DEBUG] _excluir nome = "$nome"');

    // 🔥 PRIORIDADE: conta_id (ID DA CONTA)
    final idParaExcluir = contaId.isNotEmpty ? contaId : remoteId;

    print('🔴 [DEBUG] _excluir idParaExcluir = "$idParaExcluir"');

    if (idParaExcluir.isEmpty) {
      print('🔴 [DEBUG] _excluir: idParaExcluir VAZIO!');
      Toast.error(context, '❌ ID da conta não encontrado');
      return;
    }

    print('🔴 [DEBUG] _excluir: Mostrando diálogo de confirmação');
    ...
    print('🔴 [DEBUG] _excluir confirmado = $confirmado');

    if (confirmado != true) {
      print('🔴 [DEBUG] _excluir: Cancelado pelo usuário');
      return;
    }

    Toast.info(context, '🔄 Excluindo conta...');

    try {
      print(
          '🔴 [DEBUG] _excluir: Chamando deletarContaString com ID: $idParaExcluir');
      await _repository.deletarContaString(idParaExcluir);
      print('🔴 [DEBUG] _excluir: deletarContaString concluído');

      await Future.delayed(const Duration(milliseconds: 500));

      print('🔴 [DEBUG] _excluir: Aguardou 500ms, mounted = $mounted');

      if (mounted) {
        print('🔴 [DEBUG] _excluir: Chamando _carregarDados()');
        Toast.success(context, '✅ Conta excluída com sucesso!');
        await _carregarDados();
        print('🔴 [DEBUG] _excluir: _carregarDados() finalizado');
      }
    } catch (e) {
      LoggerService.error('❌ Erro ao excluir: $e');
      print('🔴 [DEBUG] _excluir ERRO: $e');
      if (mounted) Toast.error(context, '❌ Erro ao excluir: $e');
    }

    print('🔴 [DEBUG] _excluir FINALIZADO');
  }
