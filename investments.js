...
// BRAPI CONFIG
// ============================================
const BRAPI_TOKEN = '152PAY2RUDHE9BwzBB9NLj';
...
function showToast(message, type = 'success') {
    ...
    toast.style.cssText = `
        ...
        border-left: 4px solid ${colors[type] || '#6C5CE7'};
        border-radius: 8px;
        padding: 12px 16px;
...
// ============================================
// ATUALIZAR VALOR TOTAL (COMPRA/VENDA)
// ============================================
...
// ============================================
// SALVAR OPERAÇÃO - CORRIGIDO (VERSAO FINAL)
// ============================================
async function saveOperation(e) {
    ...
    try {
        ...
        console.log('📊 Dados da operação:', { ticker, type, quantity, unitPrice, totalValue, date, assetClass, note });
        ...
        if (!quantity || quantity <= 0) {
            showToast('❌ Digite uma quantidade válida (ex: 100 ou 1,5)', 'error');
            btn.disabled = false;
            btn.innerHTML = originalText;
            return;
        }
        ...
        if (!unitPrice || unitPrice <= 0) {
            showToast('❌ Digite um preço unitário válido (ex: 10,50)', 'error');
            btn.disabled = false;
            btn.innerHTML = originalText;
            return;
        }
        ...
        console.log('📊 Enviando para Supabase:', JSON.stringify(data, null, 2));
        ...
        console.error('❌ Erro detalhado ao salvar operação:', error);

        let errorMsg = 'Erro ao salvar operação';
        if (error.code === 'PGRST204') {
            errorMsg = 'Erro: Coluna não encontrada.';
        } else if (error.code === '42501') {
            errorMsg = 'Erro: Permissão negada.';
        } else if (error.code === '23502') {
            errorMsg = 'Erro: Campo obrigatório faltando.';
...
async function saveDividend(e) {
    ...
    try {
        ...
        console.log('📊 Dados do provento:', { ticker, quantity, unitValue, totalValue, date, type, note });
        ...
        if (!unitValue || unitValue <= 0) {
            showToast('❌ Digite um valor por unidade válido (ex: 0,50)', 'error');
            btn.disabled = false;
            btn.innerHTML = originalText;
            return;
        }
        ...
        console.log('📊 Enviando provento para Supabase:', JSON.stringify(data, null, 2));
...
async function fetchQuotes() {
    ...
    try {
        ...
        for (let i = 0; i < unique.length; i += batchSize) {
            ...
            const url = `https://brapi.dev/api/quote/${encodeURIComponent(batch.join(','))}`;
...
function updateSummary() {
    ...
    console.log('📊 Resumo:', { total, invested, gain, returnPct, totalDividends });
...
// ============================================
// RENDER CARTEIRA - TABELA COM CORES
// ============================================
...
function renderChart() {
    ...
    const gainColors = data.gain.map(value =>
        value >= 0 ? '#00B894' : '#FF7675'
    );
...
function buildChartData() {
    if (allTransactions.length === 0) {
        return { labels: ['Agora'], invested: [0], gain: [0] };
    }
    ...
    for (const p of positions) {
        currentPrices.set(p.ticker, p.averageCost || 0);
    }
    ...
    for (let i = 0; i < allMonths.length; i++) {
        ...
        for (const tx of month.transactions) {
            ...
            const total = Number(tx.total_value) || qty * price;
        ...
        const monthLabel = month.date.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' });
    ...
    console.log('📊 Dados do gráfico (todos os meses):', { labels, invested, gain });
    return { labels, invested, gain };
}
...
console.log('✅ Investments.js carregado com sucesso!');